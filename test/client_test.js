...
describe('Client', () => {
  ...
  describe('#_parseReport()', () => {
    it('should properly parse LCOV reports', async () => {
      ...
      expect(job.sourceFiles[0].name).to.equal(path.join('src', 'client.js'));
      ...
      expect(job.sourceFiles[1].name).to.equal(path.join('src', 'configuration.js'));
      ...
      expect(job.sourceFiles[2].name).to.equal(path.join('src', 'git_commit.js'));
