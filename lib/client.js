...
exports.Client = class Client extends EventEmitter {
  ...
  async upload(coverage, configuration = null) {
    ...
    if (!report.length) throw new Error('The specified coverage report is empty.');

    let job;
    if (report.substring(0, 5) == '<?xml' || report.substring(0, 10) == '<coverage') {
      const {parseReport} = require('./parsers/clover');
      job = await parseReport(report);
    }
    else {
      let token = report.substring(0, 3);
      if (token == 'TN:' || token == 'SF:') {
        const {parseReport} = require('./parsers/lcov');
        job = await parseReport(report);
      }
    }

    if (!job) throw new Error('The specified coverage format is not supported.');
