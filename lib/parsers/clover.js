...
export async function parseReport(report) {
    ...
    if (!xmlDoc.coverage || typeof xmlDoc.coverage != "object")
        throw new TypeError("The specified Clover report is invalid.");
    const projects = findElements(xmlDoc.coverage, "project");
    if (!projects.length)
        throw new TypeError("The specified Clover report is empty.");
    ...
    for (const pkg of findElements(projects[0], "package"))
        for (const file of findElements(pkg, "file")) {
            ...
            if (!sourceFile.length)
                throw new TypeError(`Invalid file data: ${JSON.stringify(file)}`);
            const source = await readFile(sourceFile, "utf8");
            const coverage = new Array(source.split(/\r?\n/).length).fill(null);
            for (const line of findElements(file, "line")) {
                if (getAttribute(line, "type") != "stmt")
                    continue;
                ...
                if (Number.isNaN(lineNumber) || Number.isNaN(executionCount))
                    throw new TypeError(`Invalid line data: ${JSON.stringify(line)}`);
